import pathlib
from ..datasets.image_mask_types import ImageMaskRecord
from ...common import fs


def from_image_mask_json(root: pathlib.Path | str, json_name: str | None = None) -> list[ImageMaskRecord]:
    """数据集解析器。
    ...
    if not json_path.exists():
        raise FileNotFoundError(f"Image-mask JSON not found at: {json_path}")

    raw_data = fs.load_json(json_path)

    records: list[ImageMaskRecord] = []
