...
from .image_mask_types import ImageMaskRecord, ImageMaskItem
from ...common import fs, image
DataCollectorFunc = Callable[..., list[ImageMaskRecord]]
...
class ImageMaskDataset(Dataset):
    def __init__(self,
        ...
        """
        Args:
            root_dir (str | pathlib.Path): 数据集根目录。
            collector_fn (DataCollectorFunc): 映射函数。
            collector_kwargs (dict[str, Any]| None): 映射函数参数。
            transform (A.Compose|None): Albumentations 增强流水线。
        """
        ...
        self.data_records: list[ImageMaskRecord] = collector_fn(self.root_dir, **kwargs)
    ...
    def __getitem__(self, idx: int) -> ImageMaskItem:
