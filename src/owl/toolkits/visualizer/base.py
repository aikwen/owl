...
import torch
from ..data.datasets.image_mask_types import ImageMaskBatch
...
class OwlVisualizer(ABC):
    ...
    @abstractmethod
    def __call__(self, batch_data: ImageMaskBatch, outputs: ModelOutput, dataset_name: str):
        """执行可视化与保存操作

        Args:
            batch_data (ImageMaskBatch): 当前批次的数据，包含 tp_name 等元数据。
