...
from ..toolkits.data.datasets.image_mask_types import ImageMaskBatch
from ..toolkits.model.base import OwlModel
from ..toolkits.model.types import Metrics as ModelMetrics
from ..toolkits.criterion.types import Metrics as CriterionMetrics
from ..toolkits.data.batch import move_batch_to_device
...
class TrainStepPipeline(StateMachine):
    ...
    def __init__(
        ...
        self.ctx_batch: ImageMaskBatch | None = None
        self.ctx_outputs: Any = None
        self.ctx_model_metrics: ModelMetrics = {}
        self.ctx_loss: torch.Tensor | None = None
        self.ctx_criterion_metrics: CriterionMetrics = {}
    ...
    def on_event_forward(self):
        ...
        self.ctx_model_metrics = self.ctx_outputs.get("metrics", {})
    ...
    def on_event_compute_loss(self):
        ...
        if isinstance(loss, torch.Tensor):
            self.ctx_loss = loss
            self.ctx_criterion_metrics = {}
        else:
            self.ctx_loss = loss["loss"]
            self.ctx_criterion_metrics = loss.get("metrics", {})
    ...
    def do_step_flow(
        ...
        Args:
            batch_data (ImageMaskBatch): 原始批次数据。
        ...
        batch_data = move_batch_to_device(
            batch_data,
            self.device,
        ...
        return {
            ...
            "metrics": {
                "loss": self.ctx_criterion_metrics,
                "model": self.ctx_model_metrics,
            }
        }